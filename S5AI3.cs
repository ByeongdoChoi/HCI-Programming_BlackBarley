...
public class S5AI3 : MonoBehaviour
{
    ...
    void Start()
    {
        cloud = GameObject.Find("5AI_car3");     //AI자동차의 객체 이름 찾기
    }

    void FixedUpdate()
    {

        dt = Vector3.Distance(car.transform.position, Maincar.transform.position);  //AI 자동차와 신호등의 거리 계산
        ...
        if (dt <= 80)    //Main 자동차와 AI 자동차의 거리가 80 이하가 될때
        {
            speed = 30.0f;  //스피드 30
